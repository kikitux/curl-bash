...
mkdir -p /etc/nomad.d
curl -o /etc/systemd/system/nomad.service https://raw.githubusercontent.com/kikitux/curl-bash/master/nomad-server/nomad.service

# if we have WAN_JOIN then we are on the 2nd dc
if [ "${WAN_JOIN}" ] ; then
  curl -o /etc/nomad.d/server.hcl https://raw.githubusercontent.com/kikitux/curl-bash/master/nomad-server/nomad.d/server-dc2.hcl
  sed -i "s/192.168.56.20/${WAN_JOIN}/g" /etc/nomad.d/*.hcl
  # remove double quotes "" -> "
  sed -i -e "s/\"\"/\"/g" /etc/nomad.d/*.hcl
# else we are on the 1st dc
else
  curl -o /etc/nomad.d/server.hcl https://raw.githubusercontent.com/kikitux/curl-bash/master/nomad-server/nomad.d/server-dc1.hcl
fi
...
if [ "${RETRY_JOIN}" ]; then
  sed -i -e "s/retry_join = \[ \"localhost\" \]/retry_join = \[ \"${RETRY_JOIN}\" \]/g" /etc/nomad.d/*.hcl
  # remove double quotes "" -> "
  sed -i -e "s/\"\"/\"/g" /etc/nomad.d/*.hcl
else
  sed -i -e "s/retry_join = \[ \"localhost\" \]//g" /etc/nomad.d/*.hcl
fi
